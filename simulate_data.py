...
class Etalon:
    def __init__(self, thickness:"mm", n):
        ...
        Args
        ----
        d (float) : thickness of the etalon 
        n (float) : index of refraction of the etalon
        """
        self._n = n
        self._d = thickness
...
class Interferometer:
    ...
    def __init__(self, etalon, tau:"ns"=.1):
        ...
        Args
        ----
        etalon (Etalon) : the etalon used in the interferometer, provides VPF
        tau (float) : the time resolution of the streak camera, determined by the
    ...
    def _interfear_ray(self, ray):
        ...
        ray.add_delta(np.linspace(0,100,_delta_shape).reshape(_delta_shape,1))
    ...
    def output(self, ray, target, noise=False):
        ...
        Args
        ----
        ray (Ray class) : the input ray
        target (Target class) : target containing the velocity profile
        noise (bool) : (optional) add in detector noise to the generated image
        """
...
def spatial_var_step(a:"angle", t:"ns", y:"um", max_velocity:"um/ns"=1):
    ...
    Args
    ----
    a (float) : the slope of the spatially varying profile 
    t (float) : the time (in ns) at which to evaluate the velocity 
    y (float) : the spatial location at which to evaluate the velocity 
    max_velocity (float) : the maximum velocity of the shock
...
def sin_step(freq, amp, t:"ns", y:"um", max_velocity:"um/ns"=1):
    ...
    Args
    ----
    freq (float) : the frequency of the spatially varying profile 
    amp (float) : the amplitude of oscillations
    t (float) : the time (in ns) at which to evaluate the velocity 
    y (float) : the spatial location at which to evaluate the velocity 
    max_velocity (float) : the maximum velocity of the shock
...
def reference_shot(save=False, noise=False):
    ...
    Args
    ----
    save (bool) : (optional) save the generated image
    noise (bool) : (optional) add in detector noise 
